...
# spec-loop.sh — Thin CI/CD wrapper for /spec-loop
# All business logic lives in skills/spec-loop/SKILL.md
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    *) echo "Usage: spec-loop.sh --spec-name <name> [--max-iterations N]"; exit 1 ;;
  esac
done
...
claude -p "Run /spec-loop for spec '$SPEC_NAME' with max iterations $MAX_ITERATIONS."
